  ...
  return new Promise((resolve, reject) => {
    ...
    `)
      ...
      .then(({data: {sections, subsections, items}}) => {
        ...
        const itemsForNews = normalizedItems.filter((item) => itemsSlugsToDisplayInNews.includes(item.slug));
        ...
        normalizedSubsections.forEach((subsection) => {
          locales.forEach(locale => {
            const currentSection = normalizedSections.filter((section) => (
              section.slug === subsection.section
            ))[0];

            const subsectionItems = normalizedItems.filter((item) => (
              item.subsection === subsection.slug && item.lifeCycleState === "on"
            ));
